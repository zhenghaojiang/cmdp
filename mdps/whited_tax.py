...
class MDPWhited_tax(MDP, ABC):
    ...
    def step(self, action: Union[float, np.ndarray]):
        ...
        cash = 0.8 * self.config['pi_fn'](k_curr, z_curr, theta) + 0.2 * delta * k_curr - \
            self.config['psi_fn'](i_curr, k_curr) - i_curr
...
class cMDPWhited_tax(cMDP, ABC):

    def __init__(self, config: Dict[str, Any] = None):
        if config is None:
            config = {}
        config.setdefault('mdp_type', MDPWhited_tax)
    ...
    def unpack_env_config(self, config):
        ...
        env_config.setdefault('initial_state', (1., 1.))
